...
// Basic Medical Information lookup (Legacy from original server.js)
                ...
                if (times.some(t => t.toUpperCase().includes(currentTime) || currentTime.includes(t.toUpperCase()))) {
                    ...
                    if (twilioClient) {
                        await twilioClient.messages.create({
                            body: messageBody,
                            from: process.env.TWILIO_WHATSAPP_NUMBER || 'whatsapp:+14155238886', // Twilio Sandbox
...
app.listen(PORT, () => {
  logger.info(`TABEEB AI server running on port ${PORT}`);
});
